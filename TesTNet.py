...
class Network:
    def __init__(self,sizes,num=0):
        ...
        self.num_layers = 3
        self.sizes = sizes
        self.biases = [[num for x in range(0,y)] for y in sizes[1:]]
        self.weights = [[[num for prevneuron in range(0,sizes[i-1])]for neuron in range(0,sizes[i])]
                        for i in range(1,3)]
    ...
    def mutate(self):
            ...
            for neuron in layer:
                ...
                if i == 0:
                    for weight in neuron:
                        weight+= random.randint(-1,1)*0.5 * weight
                        if weight > 5:
                            weight = 5
                        if weight < -5:
                            weight = -5
        for layer in self.biases:
            for bias in layer:
                i = random.randint(0,1000)
                if i == 0:
                    bias += random.randint(-1,1) * 0.5 * bias
    def tolog(self):
        result = ""
        for layer in self.weights:
            for neuron in layer:
...
if __name__  ==  '__main__':
    net1 =Network([2,5,3],num=1)
    net2 = Network([2,5,3],num=2)
    children = net1.reproduction(net2)
    print(children[0].weights)
